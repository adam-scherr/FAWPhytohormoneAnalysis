...
#BA Scatter Plots
BA_point1 <- ggplot(prelim_data, aes(x= microbe_level, y = ba_avg, color = diet_type))+
...
BA_point2 <- ggplot(prelim_data, aes(x= microbe_level, y = ba_avg, color = diet_type))+
  geom_pointrange(aes(ymin = ba_low, ymax = ba_high),
                  position=position_jitter(width=0.2), size =1.5)+
  scale_color_manual(values = c("darkmagenta", "chocolate1","cyan", "sienna4"), name="Diet Type")+
  labs(x= "", y= "BA Concentration (ng/g)")+
...
BA_point3 <- ggplot(prelim_data, aes(x= microbe_level, y = ba_avg, color = diet_type))+
  geom_pointrange(aes(ymin = ba_low, ymax = ba_high),
                  position=position_jitter(width=0.2), size =1.5)+
  scale_color_manual(values = c("darkmagenta", "chocolate1","cyan", "sienna4"), name="Diet Type",
                     labels =c("Benzoic Acid", "Control", "Indoleacetic Acid", 
                               "Salicylic Acid"))+
  labs(x= "", y= "BA Concentration (ng/g)")+
  theme_clean(base_size=14)
BA_point3
...
##let's see if we can make a nice box and whisker combined with scatter plot
ordered_data <- prelim_data
ordered_data$diet_type <- factor(ordered_data$diet_type, 
                                 levels = c("control", "benzoic", "SA", "IAA"))

#BA Box and whisker
BA_box1 <- ggplot(prelim_data, aes(x = microbe_level, y= ba_avg,
                                  color = diet_type))+
  geom_boxplot(width=0.8, aes(x=factor(diet_type, level = level_order)))+
  geom_jitter(size = 3, alpha=0.5, position = position_jitter(width=0.1, seed = 0))+
  scale_color_manual(values = c("darkmagenta", "chocolate1","cyan", "sienna4"), name="Diet Type")
  
BA_box1

BA_box2 <- ggplot(ordered_data, aes(x = microbe_level, y= ba_avg,
                                   color = diet_type))+
...
BA_box3 <- ggplot(prelim_data, aes(x = diet_type, y = ba_avg, color = microbe_level))+
  geom_boxplot(width=0.8)+
  geom_point(size = 3, alpha = 0.5, position=position_dodge(width=0.8))
BA_box3

#make a new dataframe with fused microbe_level and diet_type
library(tidyr)
fused_col <- prelim_data %>%
  unite("fused", microbe_level:diet_type, remove=FALSE)

fused_col$fused <- factor(fused_col$fused, 
                                levels = c("axenic_control", "nonaxenic_control",
                                           "axenic_benzoic", "nonaxenic_benzoic",
                                           "axenic_SA", "nonaxenic_SA",
                                           "axenic_IAA", "nonaxenic_IAA"))
levels(fused_col$fused)

BA_box4 <- ggplot(fused_col, aes(x = fused, y= ba_avg, color=microbe_level))+
  geom_boxplot(width=0.8)+
  geom_point(data=fused_col, aes(x= fused, y= ba_avg, fill = microbe_level))
BA_box4

BA_box5 <- ggplot(fused_col, aes(x = fused, y= ba_avg, color=microbe_level))+
  geom_boxplot(width=0.8)+
  geom_point(aes(fill = diet_type))
BA_box5

library(ggh4x)
BA_box6 <- ggplot(prelim_data, aes(x = interaction(diet_type, microbe_level, sep = "!"),
                                   y= ba_avg, color = diet_type))+
  geom_boxplot()+
  scale_x_discrete(guide = guide_axis_nested(delim = "!"), name = "microbe_level")
BA_box6
...
####plots that you like: SA_point1, BA_box3, BA_box6
  #actually, definitely use BA_box3; it's beautiful now!
...
#we did not meet assumptions for Tukey-HSD, so let's try Adonis pairwise comparison
  #as our post-hoc tests
# View(iris)
# pairwise_adonis(x=perm.data[,5:7], factors= perm.data$microbe_level,
#                 sim_method = "euclidean", p_adjust_m = "bonferroni")
# 
# # install.packages("remotes")
# # remotes::install_github("gauravsk/ranacapa")
# # library(remotes)
# # library("gauravsk/ranacapa")
# library(vegan)
