...
BA_box <- ggplot(prelim_data, aes(x = diet_type, y = ba_avg, fill = microbe_level))+
  ...
  geom_point(size = 4, alpha = 0.82, position=position_dodge(width=0.8), aes(color= microbe_level)) +
  scale_fill_manual(values = c("#dd3497", "#7a0177"), name = "", 
                     labels = c("Axenic", "Nonaxenic"))+
  scale_color_manual(values = c("#dd3497", "#7a0177"), name = "", 
                     labels = c("Axenic", "Nonaxenic"))+
