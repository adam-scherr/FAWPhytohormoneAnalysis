...
####Two-way ANOVA for BA-spiked and unspiked salivary glands####
unspikedandba.data <- data.1 %>%
  filter(diet_type != "indole") %>%
  filter(diet_type != "salicylic")

unspikedandba.data$diet_type <- factor(data.1$diet_type, 
                           levels = c("control", "benzoic", "salicylic", "indole"))
...
par(mfrow=c(2,2))
plot(unspikedandba.model)
  #not the best normality, but variance looks pretty good
summary(unspikedandba.model)
  #significance for microbe_level AND diet_type (and nearly their interaction, p=0.0524)

#post hoc with TukeyHSD
TukeyHSD(unspikedandbaba.model, conf.level=0.95)

#trying it with emmeans and cld
library(lme4)
library(emmeans)
library(multcomp)

cld(emmeans(unspikedandba.model, pairwise ~ microbe_level*diet_type, adjust = "tukey"))
emmeans :: emmeans(unspikedandba.model, ~ microbe_level*diet_type) %>%
  multcomp :: cld(adjust = "Tukey", details = TRUE)
  #yields an ordered output of what is significant to what

# emmeans(aov2_arbuscular, 
