...
ba_box <- ggplot(ba.data, aes(x = microbe_level, y=ba_avg, fill=microbe_level)) +
  ...
  scale_fill_manual(values = c("#9ecae1", "#3182bd", "#08306b"), name = "")+
  scale_color_manual(values = c("#9ecae1", "#3182bd", "#08306b"), name="") +
...
sa_box <- ggplot(sa.data, aes(x = microbe_level, y = sa_avg, fill = microbe_level)) +
  geom_boxplot(width=0.8, color = "black", alpha = 0.82) +
  geom_point(size = 4, alpha = 1, position=position_dodge(width=0.8), aes(color = microbe_level))+
  scale_fill_manual(values = c("#a1d99b", "#238b45", "#00441b"), name = "")+
  scale_color_manual(values = c("#a1d99b", "#238b45", "#00441b"), name = "")+
...
iaa_box <- ggplot(iaa.data, aes(x=microbe_level, y = iaa_avg, fill = microbe_level))+
  geom_boxplot(width=0.8, color = "black", alpha = 0.82) +
  geom_point(size = 4, alpha = 1, position=position_dodge(width=0.8), aes(color = microbe_level))+
  scale_fill_manual(values = c("#fdae6b", "#d94801", "#7f2704"), name="")+
  scale_color_manual(values = c("#fdae6b", "#d94801", "#7f2704"), name="")+
...
unspiked_ba_box <- ggplot(unspiked.ba.data, aes(x = microbe_level, y=ba_avg, fill=microbe_level)) +
  ...
  scale_color_manual(values = c("#9ecae1", "#3182bd", "#08306b"))+
  labs(x = "Microbe Treatment", y = "BA Concentration (ng/g)")+
  theme_clean() +
  theme(axis.text = element_text(size =15),
        panel.grid.major = element_line(size = 2),
...
TukeyHSD(unspiked.ba.model, conf.level = 0.95)
  #SIGNIFICANCE for interaction between E. mund and axenic (p=0.0279)
TukeyHSD(unspiked.sa.model, conf.level=0.95) #no significance
TukeyHSD(unspiked.iaa.model, conf.level=0.95) #no significance
...
unspikedandsa_box <- ggplot(unspikedandsa.data, aes(x = diet_type, y = sa_avg, fill = microbe_level))+
  ...
  geom_point(size = 4, alpha =1, position=position_dodge(width=0.8), aes(color=microbe_level))+
  scale_fill_manual(values = c("#a1d99b", "#238b45", "#00441b"))+
  scale_color_manual(values =c("#a1d99b", "#238b45", "#00441b"))+
...
unspikedandiaa_box <- ggplot(unspikedandiaa.data, aes(x = diet_type, y = iaa_avg, fill = microbe_level))+
  ...
  geom_point(size = 4, alpha =1, position=position_dodge(width=0.8), aes(color=microbe_level))+
  scale_fill_manual(values = c("#fdae6b", "#d94801", "#7f2704"))+
  scale_color_manual(values =c("#fdae6b", "#d94801", "#7f2704"))+
...
summary(unspikedandba.model)
  #significance for microbe_level AND diet_type (and nearly their interaction, p=0.0524)

summary(unspikedandsa.model)
  #significance for diet_type only

summary(unspikedandiaa.model)
  #significance for diet_type, microbe_level, AND the interaction betwixt them
...
# Axenic        control      124 1148 12    -2378     2626  a    
# Frass Com.    control      232 1148 12    -2271     2734  a    
# E. mundtii    control      275 1148 12    -2228     2777  a    
# Axenic        benzoic     7492 1148 12     4990     9994   b   
# Frass Com.    benzoic     8007 1148 12     5505    10510   bc  
# E. mundtii    benzoic    13326 1148 12    10824    15828    c 
...
#output:
# microbe_level diet_type  emmean   SE df lower.CL upper.CL .group
# Axenic        control     -40.8 4766 12   -10424    10342  a    
# Frass Com.    control      50.9 4766 12   -10332    10434  a    
# E. mundtii    control      63.8 4766 12   -10320    10447  a    
# Frass Com.    salicylic 19531.1 4766 12     9148    29914  ab   
# Axenic        salicylic 22148.2 4766 12    11765    32531  ab   
# E. mundtii    salicylic 26865.1 4766 12    16482    37248   b 
...
#output:
# microbe_level diet_type  emmean   SE df lower.CL upper.CL .group
# E. mundtii    control      41.3 3341 12    -7238     7320  a    
# Frass Com.    control     117.0 3341 12    -7162     7396  a    
# Axenic        control     144.9 3341 12    -7134     7424  a    
# Frass Com.    indole    12580.2 3341 12     5301    19859  ab   
# E. mundtii    indole    26121.6 3341 12    18843    33400   b   
# Axenic        indole    42574.6 3341 12    35296    49853    c  
